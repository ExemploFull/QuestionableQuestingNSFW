...
// Write minified version without spaces/tabs
fs.writeFileSync(jsonMinPath, JSON.stringify(json));

// Write pretty version with tab formatting
...
// check for broken plugins safely
for (let language in languages) {
  const rawLangPath = path.join(RAW_PLUGIN_DIR, language.toLocaleLowerCase());
  if (!fs.existsSync(rawLangPath)) continue;
  const tsFiles = fs.readdirSync(rawLangPath);
  tsFiles
...
for (const language of Object.keys(languages)) {
  if (!pluginsPerLanguage[language]) continue;
  console.warn(
